...
typedef struct { uint64_t key; int value; } ci_int;
...
typedef struct { const char *key; int value; } ci_str;
...
typedef struct {
  uint64_t key;
  struct { char data[64]; int value; char name[24]; bool active; } value;
} ci_lv;
...
int cpfx##_int_iterate (void *h) {                                   \
  ...
  while (tpfx##_iter_valid_##E (&it)) {                              \
    s += it.ptr->value; tpfx##_iter_next_##E (t, &it);               \
  }                                                                  \
  return s;                                                          \
}                                                                    \
                                                                     \
...
size_t cpfx##_str_mixed (char *const *keys, int n) {                 \
  ...
  for (int j = 0; j < n; j++) {                                      \
    int op = j % 10;                                                  \
    E e = { keys[j], j }; E *r;                                      \
    if (op < 7) { if (!tpfx##_perform_##E (&t, &e, A##_INSERT, &r)) \
                    *r = e; }                                         \
    else if (op < 9) { tpfx##_perform_##E (&t, &e, A##_FIND, &r); } \
    else tpfx##_perform_##E (&t, &e, A##_DELETE, &r);                \
  }                                                                  \
  size_t c = tpfx##_els_count_##E (&t);                              \
  tpfx##_destroy_##E (&t); return c;                                 \
}                                                                    \
...
int cpfx##_lv_lookup (void *h, const uint64_t *keys, int n) {        \
  ...
  for (int i = 0; i < n; i++) {                                      \
    ...
    E *r;                                                             \
    if (tpfx##_perform_##E (t, &e, A##_FIND, &r))                    \
      s += r->value.value;                                            \
  }                                                                  \
  return s;                                                          \
}                                                                    \
